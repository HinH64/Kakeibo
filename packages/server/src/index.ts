...
app.get("/api/transactions/with-details", async (c) => {
  const filter: any = {};
  if (c.req.query("accountId")) filter.accountId = c.req.query("accountId");
  if (c.req.query("categoryId")) filter.categoryId = c.req.query("categoryId");
  if (c.req.query("type")) filter.type = c.req.query("type");
...
app.get("/api/transactions/spending-by-category", async (c) => {
...
app.get("/api/transactions/month-stats", async (c) => {
  const from = c.req.query("from")!;
  const to = c.req.query("to")!;
...
app.get("/api/transactions/monthly-trend", async (c) => {
